...
REVIEWED_IN_WORKTREE = (
    ...
    # REVERTING the end-anchored active region, 2026-08-25. The experiment is
    # withdrawn: the canonical region is once again derived FORWARD from the
    # RIGHT_UP master timeline, exactly as before it.
    #
    # This entry is a revert, not a change. `train_structure.py`,
    # `global_fusion.py` and `run_global_count.py` are restored byte-for-byte to
    # 5e4bcf1, so the diff the guard sees is against HEAD (which still carries
    # the experiment) rather than against the behaviour that ships. Verify with
