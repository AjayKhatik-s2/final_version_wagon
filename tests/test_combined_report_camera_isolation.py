...
def _origin_of(path, tol=5.0):
    ...
    Tolerates None so it is safe inside an assertion message: those are built
    eagerly, so calling this on a missing path would crash the test while
    formatting a message for an assertion that was about to pass.
    """
    if not path:
        return None
    m = _mean_of(path)
    ...
    def test_missing_evidence_yields_nothing_rather_than_a_substitute(self):
        ...
        with tempfile.TemporaryDirectory() as root:
            ...
            self.assertIsNone(got,
                              f"LEFT_UP_TOP fabricated {_origin_of(got)} "
                              f"({got})")
